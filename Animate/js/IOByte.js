...
const IOByte = new function(){
	...
	this.putBytes = function(src, dest, pointer){
		for(var i=0; i<src.length&&i+pointer<dest.length; i++){
			dest[i+pointer] = src[i];
		}
		return dest;
	}
	...
	this.bytesAsString = function(bytes){
		...
		for(var i=0; i<bytes.length; i++)
			if(validChar(bytes[i]))
				str += String.fromCharCode(bytes[i]);
			else
				str += "";
		return str;
	}
	this.stringAsBytes = function(string){
		var bytes = [];
	...
	this.intToHex = function(val){
		return val.toString(16);
	}
	this.hexToInt = function(val){
