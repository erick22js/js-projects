...
var BLENDCOLOR = [1, 1, 1, 1];
var FOGMIN = 32;
var FOGMAX = 512;
var FOGCOLOR = [0, 1, 1, .5];
var FOGCOLORDIF = [1, 0, 0];
...
var gl = new function () {
	...
	function drawPixel(x, y, r, g, b, a, depth) {
		if (x > -1 && x < WIDTH && y > -1 && y < HEIGHT)
			if (depth >= screenBuffer[~~y][~~x][4] && a > 0) {
				...
				//alert(this.BLENDRED);
				//if (this.BLENDRED) alert(this.BLENDRED);
				//screenBuffer[~~y][~~x] = [r * BLENDCOLOR[0], g * BLENDCOLOR[1], b * BLENDCOLOR[2], a * BLENDCOLOR[3], depth];
				screenBuffer[~~y][~~x] = [
					r * (1 - BLENDCOLOR[3]) + BLENDCOLOR[0] * BLENDCOLOR[3],
					g * (1 - BLENDCOLOR[3]) + BLENDCOLOR[1] * BLENDCOLOR[3],
					b * (1 - BLENDCOLOR[3]) + BLENDCOLOR[2] * BLENDCOLOR[3],
					a,
					depth
				];
			}
	...
	this.drawWall = function (xs, xe, a1, a2, d1, d2, hs, he) {
		...
		for (var x = xs < 0 ? 0 : xs > WIDTH ? WIDTH : xs; x != xe; x += add) {
			...
			for (var y = aligny < 0 ? -aligny : 0; y <= nh; y++) {
				...
				if (yn > HEIGHT) break;
				var color = TextureBuffer[~~(map[1] * this.TextureSize[1])][~~(map[0] * this.TextureSize[0])];
				drawPixel(x, yn, color[0], color[1], color[2], color[3], nd);
			}
		}
	}
	this.drawStrip = function (x, ys, u, height, depth, wallheight, s, anglcos) {
		var q = (wallheight / this.TextureSize[1]);
		var mColor = actMap.sectors[s].color;
		for (var y = ys < 0 ? 0 : ys; y <= ys + height && y <= HEIGHT; y++) {
			//(y-ys)/height
			//var map = [(u + this.TextureReposition[0] / this.TextureSize[0]) % 1, (((y - ys) / height * q) + this.TextureReposition[1] / this.TextureSize[1]) % 1];
			var map = [(u + this.TextureReposition[0]) % 1, (((y - ys) / height * q) + this.TextureReposition[1]) % 1];
			map = [map[0] < 0 ? 1 + map[0] : map[0], map[1] < 0 ? 1 + map[1] : map[1]];
			var color = getTexel(~~(map[0] * this.TextureSize[0]), ~~(map[1] * this.TextureSize[1]));
			drawPixel(x, y, color[0] * mColor[0] * anglcos, color[1] * mColor[1] * anglcos, color[2] * mColor[2] * anglcos, color[3], depth);
		}
	}
	this.drawFloorStrip = function (x, ys, d, s, h, angleX, max) {
		var y = ys;
		var actAngle = WorldMatrix.angleAdd * (y - HEIGHT * .5 - 160 * WorldMatrix.lookf);
		var mColor = actMap.sectors[s].color;
		var divCsax = 1 / Math.cos(angleX);
		while (y < max && y < HEIGHT) {
			var dp = h / Math.tan(actAngle);
			BLENDCOLOR = getFogByLevel(dp);
			var xp = Math.sin(angleX + WorldMatrix.rotation) * (dp * divCsax);
			var yp = Math.cos(angleX + WorldMatrix.rotation) * (dp * divCsax);
			var u = (xp * pamx) + WorldMatrix.z;
			var v = (yp * pamy) + WorldMatrix.x;
			...
			//drawPixel(x, y, 1, 0, 0, 1, 1);
			y++;
			actAngle += WorldMatrix.angleAdd;
		}
	}
	this.drawCeilStrip = function (x, ys, d, s, h, angleX, min) {
		...
		while (y > -1 && y > min) {
			...
			var xp = Math.sin(angleX + WorldMatrix.rotation) * (dp * divCsax);
			var yp = Math.cos(angleX + WorldMatrix.rotation) * (dp * divCsax);
			var u = WorldMatrix.z - (xp * pamx);
			var v = WorldMatrix.x - (yp * pamy);
			var color = getTexel(Math.abs(~~(u % this.TextureSize[0])), Math.abs(~~(v % this.TextureSize[1])));
			drawPixel(x, y, color[0] * mColor[0], color[1] * mColor[1], color[2] * mColor[2], color[3], d);
			//drawPixel(x, y, 0, 0, 1, 1, 1);
			y--;
			actAngle -= WorldMatrix.angleAdd;
		}
	}
...
function getFogByLevel(dist){
	var lv = dist / FOGMAX;
	return [FOGCOLOR[0], FOGCOLOR[1], FOGCOLOR[2], dist <= FOGMAX ? lv * FOGCOLOR[3] : FOGCOLOR[3]];
}
