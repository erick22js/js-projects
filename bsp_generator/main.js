...
function testInter(){
	...
	nodesL.splice(0,0,splitTree(map, null, ""));
...
setorsList.onchange = function(){
	...
	console.log(node);
	outSay.textContent += "\nQuantityLines: "+(node.lines==null?node.linesL.length+node.linesR.length:node.lines.length);
	outSay.textContent += "\nNode: "+(JSON.stringify(node, function(key, value){
		if(key=="lines"){
			var lines = [];
			for(var i=0; i<value.length; i++){
				var line = {};
				line.v1x = value[i].v1.x;
				line.v1y = value[i].v1.y;
				line.v2x = value[i].v2.x;
				line.v2y = value[i].v2.y;
